...
export default function SettingsPage() {
  ...
  const [siteUrl,   setSiteUrl]   = useState("https://viralkatta.com");
  ...
  useEffect(() => {
    fetch("/api/whatsapp-settings")
      ...
      .then(d => {
        ...
        setSiteUrl(d.siteUrl || "https://viralkatta.com");
      });
  }, []);
  ...
  async function save() {
    ...
    const res = await fetch("/api/whatsapp-settings", {
      ...
      body:    JSON.stringify({ groupLink, siteUrl }),
    });
    setSaving(false);
    if (res.ok) showToast("✅ Settings सेव्ह झाले!");
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="flex items-center gap-3 p-5 border-b border-gray-100"
            ...
            <div>
              <h2 className="font-bold text-white text-base">Site URL</h2>
              <p className="text-white/70 text-xs" style={MR}>WhatsApp share मध्ये वापरला जाणारा article link</p>
            </div>
          </div>
          <div className="p-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-5 space-y-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5" style={MR}>💬 Group Invite Link</label>
              <input type="url" value={groupLink} onChange={e => setGroupLink(e.target.value)}
